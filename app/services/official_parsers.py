...
def _table_label_pattern(label: str) -> str:
    return rf"{re.escape(label)}(?:\s*\((?!-?[0-9,])[^)]*\)\s*)*(?:\s+\d{{1,2}}(?![0-9,]))*"
...
def _extract_multi_period_row(flat_text: str, label: str) -> tuple[Optional[float], Optional[float], Optional[float]]:
    label_pattern = re.compile(_table_label_pattern(label), flags=re.IGNORECASE)
    token_pattern = re.compile(r"\(?-?[0-9][0-9,]*(?:\.[0-9]+)?\)?|—")
    best: tuple[Optional[float], Optional[float], Optional[float]] = (None, None, None)
    best_score = -1

    for match in label_pattern.finditer(flat_text):
        prefix = flat_text[max(0, match.start() - 120) : match.start()].lower()
        tail = flat_text[match.end() : match.end() + 180]
        body_match = re.match(r"(?:\s+(?:\(?-?[0-9][0-9,]*(?:\.[0-9]+)?\)?|—)){3,8}", tail)
        if not body_match:
            continue
        raw_tokens = token_pattern.findall(body_match.group(0))
        if len(raw_tokens) < 3:
            continue

        while len(raw_tokens) >= 6 and re.fullmatch(r"\d{1,2}", raw_tokens[0] or ""):
            raw_tokens = raw_tokens[1:]
        while len(raw_tokens) >= 6 and re.fullmatch(r"\d{1,2}", raw_tokens[-1] or ""):
            raw_tokens = raw_tokens[:-1]
        if len(raw_tokens) < 3:
            continue

        values = [_parse_number(token) for token in raw_tokens]
        ...
        if len(raw_tokens) >= 5:
            score += 8
        if any(token in prefix for token in ("quarter on quarter", "quarter ended", "three months ended", "q1", "q2", "q3", "q4")):
            score += 12
        if any(token in prefix for token in ("year on year", "year ended", "twelve months ended")):
            score -= 4

        if score > best_score:
            current_bn = _bn_from_millions(str(values[-1]))
            prior_bn = _bn_from_millions(str(values[0]))
            best = (current_bn, prior_bn, _pct_change(current_bn, prior_bn))
            best_score = score
...
def _extract_cash_flow_reconciliation_metrics(text: str) -> dict[str, Optional[float]]:
    ...
    for anchor in re.finditer(anchor_pattern, raw_text, flags=re.IGNORECASE):
        ...
        operating_match = _search(r"Net cash provided by(?:\s+\(used in\))?\s+operating activities\s*\$?\s*\(?([0-9,]+(?:\.[0-9]+)?)\)?", block)
        free_cash_flow_match = _search(r"Free cash flow\s*\$?\s*\(?([0-9,]+(?:\.[0-9]+)?)\)?", block)
        capex_match = _search(
            r"(?:Less:\s*)?(?:purchases of property and equipment|purchases of property, plant and equipment(?: and intangible assets)?|capital expenditures)"
            r"\s*\$?\s*\(?([0-9,]+(?:\.[0-9]+)?)\)?",
            block,
        )
        ...
        for material in ordered_materials:
            ...
            if operating_cash_flow_bn is None:
                operating_cash_flow_bn = _extract_cash_flow_narrative_metric(
                    flat_text,
                    [
                        r"(?:operating cash flow|cash flow from operations|net cash provided by(?:\s+\(used in\))?\s+operating activities|cash generated by operating activities)",
                    ],
                )
            if free_cash_flow_bn is None:
            ...
            if capital_expenditures_bn is None:
                capital_expenditures_bn = _extract_cash_flow_narrative_metric(
                    flat_text,
                    [
                        r"(?:capital expenditures|purchases of property and equipment|purchases of property, plant and equipment(?: and intangible assets)?|payments for acquisition of property, plant and equipment|payments for acquisition of property and equipment)",
                    ],
                )
...
def _extract_table_metric(
    ...
    for label in labels:
        ...
        current, prior, yoy = _extract_multi_period_row(flat_text, label)
        if current is not None:
            return (current, prior, normalize_yoy(current, prior, yoy))
